from marshmallow import Schema, ValidationError, fields, validate, validates_schema

from app.models.field import FIELD_TYPES


class PostSchema(Schema):
    label = fields.String(required=True, validate=validate.Length(min=1, max=200))
    type = fields.String(required=True, validate=validate.OneOf(FIELD_TYPES))
    order = fields.Integer(required=False, load_default=0)
    required = fields.Boolean(required=False, load_default=False)
    options = fields.List(fields.String(), required=False, allow_none=True, load_default=None)
    validation_rules = fields.Dict(required=False, allow_none=True, load_default=None)

    @validates_schema
    def validate_select_has_options(self, data, **kwargs):
        if data.get("type") == "select" and not data.get("options"):
            raise ValidationError("options is required and must be non-empty for type 'select'.", "options")
